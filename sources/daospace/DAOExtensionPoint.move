module DAOSpaceFramework::DAOExtensionPoint {
    ...
    public fun register<ExtPointT: store>(sender: &signer, name: vector<u8>, description: vector<u8>, types_d_ts:vector<u8>, dts_doc:vector<u8>, 
        ...
        let account = DAORegistry::get_daospace_signer();
        move_to(&account, Entry<ExtPointT>{
            id: copy extpoint_id, 
            name: copy name,
        ...
        move_to(&account, ExtensionPointEventHandlers<ExtPointT>{
            publish_version: Event::new_event_handle<PublishVersionEvent<ExtPointT>>(&account),
            star: Event::new_event_handle<StarEvent<ExtPointT>>(&account),
            unstar: Event::new_event_handle<UnstarEvent<ExtPointT>>(&account),
            update: Event::new_event_handle<UpdateInfoEvent<ExtPointT>>(&account),
        });
